...
from .firestore_client import _ensure_client, COL_PORTFOLIO_REPORTS, COL_TICKER_REPORTS, COL_PORTFOLIO_SUMUP, firestore, user_collection
...
def save_portfolio_report_user(uid: str, portfolio: str, report: Dict[str, Any], reference_date: Optional[str] = None, cost: Optional[float] = None) -> None:
    client = _ensure_client()
    if reference_date is None:
        reference_date = datetime.now().isoformat()
    payload = {"report": report, "cost": float(cost) if cost is not None else None, "reference_date": reference_date, "created_at": firestore.SERVER_TIMESTAMP}
    user_collection(client, uid, COL_PORTFOLIO_REPORTS).document(f"{portfolio}_risk").set(payload)


def get_portfolio_report_user(uid: str, portfolio: str, reference_date: Optional[str] = None) -> Optional[Dict[str, Any]]:
    client = _ensure_client()
    col = user_collection(client, uid, COL_PORTFOLIO_REPORTS)
    if reference_date:
        doc = col.document(f"{portfolio}_risk").get()
        if doc.exists:
            data = doc.to_dict() or {}
        ...
        if doc.exists:
            data = doc.to_dict() or {}
            return {"report": data.get("report"), "cost": data.get("cost"), "reference_date": data.get("reference_date")}
        docs = [d for d in col.stream() if d.id.startswith(f"{portfolio}__")]
        if not docs:
            return None
        docs_sorted = sorted(docs, key=lambda d: (d.to_dict() or {}).get("created_at") or datetime.min, reverse=True)
        data = docs_sorted[0].to_dict() or {}
...
def save_portfolio_sumup_user(uid: str, portfolio: str, sumup: Dict[str, Any], reference_date: Optional[str] = None, cost: Optional[float] = None) -> None:
    client = _ensure_client()
    if reference_date is None:
        reference_date = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')
    try:
        ...
        dt = datetime.utcnow()
    doc_id = f"{portfolio}_{dt.strftime('%Y_%m_%d')}"
    payload = {"sumup": sumup, "cost": float(cost) if cost is not None else None, "reference_date": reference_date, "created_at": firestore.SERVER_TIMESTAMP}
    user_collection(client, uid, COL_PORTFOLIO_SUMUP).document(doc_id).set(payload)


def get_portfolio_sumup_user(uid: str, portfolio: str, date: Optional[str] = None) -> Optional[Dict[str, Any]]:
    client = _ensure_client()
    if date is None:
        date = datetime.utcnow().strftime('%Y-%m-%d')
    try:
        ...
        return None
    doc_id = f"{portfolio}_{dt.strftime('%Y_%m_%d')}"
    doc = user_collection(client, uid, COL_PORTFOLIO_SUMUP).document(doc_id).get()
    if not doc.exists:
        return None
    data = doc.to_dict() or {}
    return {"sumup": data.get("sumup"), "cost": data.get("cost"), "reference_date": data.get("reference_date")}
