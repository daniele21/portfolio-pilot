...
export interface AlertCondition {
  id: string;
  type: 'volatility' | 'portfolio_return' | 'ticker_return';
  enabled: boolean;
  threshold: number;
  comparison: 'above' | 'below';
  period?: '30d' | '90d' | '1y'; // For volatility
  timeframe?: '1d' | '3d' | '1w' | '1m' | '3m' | 'ytd' | '1y'; // For returns
  ticker?: string; // For ticker-specific alerts
  name: string;
  description?: string;
}
