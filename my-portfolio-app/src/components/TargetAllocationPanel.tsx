...
export default function TargetAllocationPanel({ open, onClose, portfolioName, idToken, onSaved, assetTypes, riskOptions }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4">
      <div className="bg-gray-900 p-6 rounded-xl shadow-2xl w-full max-w-2xl overflow-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-indigo-300">Configure Targets for {portfolioName}</h3>
          <div className="flex items-center gap-2">
            <select value={mode} onChange={e => setMode(e.target.value as any)} className="bg-gray-800 text-gray-100 px-2 py-1 rounded">
              <option value="asset_type">By Asset Type</option>
              <option value="risk">By Risk</option>
            </select>
            <button className="px-3 py-1 rounded bg-gray-700 text-gray-200" onClick={onClose}>Close</button>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          {(availableKeys.length > 0 ? availableKeys : Object.keys(targets).slice(0,10)).map(k => (
            <div key={k} className="flex items-center gap-2">
              <div className="flex-1 text-sm text-gray-200">{k}</div>
              <input type="number" className="w-28 p-2 rounded bg-gray-800 text-gray-100 border border-gray-700 text-right" value={targets[k] ?? 0} onChange={e => updateKey(k, Number(e.target.value))} />
            </div>
          ))}
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button className="px-3 py-1 rounded bg-gray-700 text-gray-200" onClick={() => { setTargets({}); }}>Reset</button>
            <button className="px-3 py-1 rounded bg-indigo-600 text-white font-semibold" onClick={handleSave} disabled={saving}>{saving ? 'Saving...' : 'Save Targets'}</button>
          </div>
        </div>
