...
const RiskAnalysis: React.FC<RiskAnalysisProps> = ({ volatilityWindow, volatilityValue, setVolatilityWindow, presets, llmRisk }) => {
  ...
  <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 h-full max-h-[547px]">
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        {/* Compact LLM risk analysis */}
        {llmRisk && typeof llmRisk === 'object' && (
          <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 rounded-lg p-4 border border-purple-400/20">
            <LLMRiskSummary llmRisk={llmRisk} />
          </div>
        )}

        {/* Compact Volatility Card */}
        <div className={`${riskInfo.bgColor} rounded-xl p-4 border border-white/10`}>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <select
                  ...
                  className="bg-white/10 border border-white/20 rounded px-2 py-1 text-xs text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 hover:bg-white/15 transition-colors"
                >
                  {presets.map(preset => (
